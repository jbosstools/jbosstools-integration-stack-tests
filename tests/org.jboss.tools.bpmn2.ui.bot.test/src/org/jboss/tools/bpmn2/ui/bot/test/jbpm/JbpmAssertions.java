...
public class JbpmAssertions {
	...
	public static void assertProcessInstanceCompleted(ProcessInstance process, KieSession ksession) {
		assertEquals(process.getProcessName() + " wasn't completed.", ProcessInstance.STATE_COMPLETED,
				process.getState());
			...
			if (node.getName().equals(nodeName)) {
				if (node.getIncomingConnections().get("DROOLS_DEFAULT").size() != num) {
					fail("Expected incomming connections: " + num + " - found "
							+ node.getIncomingConnections().get("DROOLS_DEFAULT").size());
				} else {
					break;
				}
			}
			...
			if (node.getName().equals(nodeName)) {
				if (node.getOutgoingConnections().get("DROOLS_DEFAULT").size() != num) {
					fail("Expected outgoing connections: " + num + " - found "
							+ node.getOutgoingConnections().get("DROOLS_DEFAULT").size());
				} else {
					break;
				}
			}
