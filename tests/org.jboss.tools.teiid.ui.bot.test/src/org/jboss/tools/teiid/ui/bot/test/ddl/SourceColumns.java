...
public class SourceColumns {
	...
	private static final String PROJECT_NAME = "SourceColumns";
	private static final String NAME_SOURCE_MODEL = "SourceModelColumns";
	private static final String NAME_VDB = "SourceModelColumnsVDB";
	private static final String NAME_ORIGINAL_DYNAMIC_VDB = NAME_VDB + "-vdb.xml";
	
	private static final String NAME_GENERATED_DYNAMIC_VDB = "SourceModelColumnsVDBgenerated-vdb.xml";
	...
	private void checkImportedModel(){				
	    ...
	    // check tColumn2
 		...
 		collector.checkThat("Native type for tColumn3 is badly set", tableEditor.getCellText(1,"tColumn3", "Native Type"),
 		    	is("tColumn3NT"));
 		collector.checkThat("Length of tColumn3 is badly set", tableEditor.getCellText(1,"tColumn3", "Length"),
 	    		is("123"));
		...
		collector.checkThat("Native type for pColumn2 is badly set", tableEditor.getCellText(1,"pColumn2", "Native Type"),
		    	is("pColumn2NT"));
 		collector.checkThat("Length of pColumn2 is badly set", tableEditor.getCellText(1,"pColumn2", "Length"),
 	    		is("456"));
	...
	private void checkExportedFile(String contentFile){
		collector.checkThat("tColumn1 is badly set", contentFile,
				new StringContains("tColumn1 bigdecimal(1) NOT NULL OPTIONS(NAMEINSOURCE 'tColumn1Source', NATIVE_TYPE 'tColumn1NT', ANNOTATION 'tColumn1 description')"));
		collector.checkThat("tColumn2 is badly set", contentFile,
				new StringContains("tColumn2 float OPTIONS(NAMEINSOURCE 'tColumn2Source', NATIVE_TYPE 'tColumn2NT', ANNOTATION 'tColumn2 description')"));
		collector.checkThat("tColumn3 is badly set", contentFile,
				new StringContains("tColumn3 string(123) OPTIONS(NAMEINSOURCE 'tColumn3Source', NATIVE_TYPE 'tColumn3NT', ANNOTATION 'tColumn3 description')"));
		collector.checkThat("pColumn1 is badly set", contentFile, 
				new StringContains("pColumn1 biginteger(4000) NOT NULL OPTIONS(NAMEINSOURCE 'pColumn1Source', NATIVE_TYPE 'pColumn1NT', ANNOTATION 'pColumn1 description')"));
		collector.checkThat("pColumn2 is badly set", contentFile, 
				new StringContains("pColumn2 string(456) OPTIONS(NAMEINSOURCE 'pColumn2Source', NATIVE_TYPE 'pColumn2NT', ANNOTATION 'pColumn2 description')"));				
	}
}
