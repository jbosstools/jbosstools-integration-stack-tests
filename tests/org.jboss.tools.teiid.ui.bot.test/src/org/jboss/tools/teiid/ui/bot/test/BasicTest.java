...
public class BasicTest {
	...
	//BooksInput, BooksUpdate, ISBNInput : mns:ISBNType, putResults : mns:putResultsType
	private static String[] virtDocs = new String[]{"BooksInput", "BooksUpdate", "ISBNInput : mns:ISBNType", "putResults : mns:putResultsType"};
	...
	private void output(int j, String elem){
		try{
    		...
    		PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("/home/lfabriko/tmp/myfile.txt", true)));
    		out.println(MODEL+j + ", " + elem);
    		s = new DefaultTreeItem(PROJECT, MODEL+j+".xmi", el, el2).getItems().size();
    		for (int i = 0; i < s; i++){
    			String data = new DefaultTreeItem(PROJECT, MODEL+j+".xmi", el, el2).getItems().get(i).getText();
    			out.println(data);
    		} 
    	        out.close();
    	}catch(IOException e){
