...
/**
 * Test E2eAudioBooksVdbExecutionTestScript_Teiid7.odt Prerequisites: BooksWithSOAPEncoding.xsd, BookDatatypes.xsd,
 * soap_encoding.xsd
 * 
 * @author lfabriko
 *
...
public class E2eAudioBooksVdbExecutionTest extends SWTBotTestCase {
	...
	public void test03() {
		try {
			...
			String[] tables1 = {
				"PUBLIC/PUBLIC/TABLE/AUTHORS",
				"PUBLIC/PUBLIC/TABLE/BOOKS",
				"PUBLIC/PUBLIC/TABLE/PUBLISHERS" };
			...
			String[] tables2 = {
				"PUBLIC/PUBLIC/TABLE/AUTHORS",
				"PUBLIC/PUBLIC/TABLE/BOOKS",
				"PUBLIC/PUBLIC/TABLE/BOOK_AUTHORS" };
			...
			// System.out.println(new SWTWorkbenchBot().getFocusedWidget().getClass());//class
			...
			new DefaultTreeItem(new DefaultTree(0), PROJECT_NAME, SOURCE_MODEL_2 + ".xmi", "BOOKS", "FK_PUBLISHER")
					.select();
	...
	public void test06() {
		try {
			...
			modelView.newBaseTable(PROJECT_NAME, BOOKSINFO_MODEL + ".xmi", BOOKINFO_VIRTUAL_TABLE_, false);

			// open transformation diagram of bookInfo
			modelView.openTransformationDiagram(PROJECT_NAME, BOOKSINFO_MODEL + ".xmi", BOOKINFO_VIRTUAL_TABLE_);

			// transformation - add vBooks_B_BA.BOOKS and vBooks_A_P.PUBLISHERS
			modelView.addTransformationSource(PROJECT_NAME, VIRTUAL_B_BA + ".xmi", "BOOKS");
			modelView.addTransformationSource(PROJECT_NAME, VIRTUAL_A_P + ".xmi", "PUBLISHERS");
			...
			criteriaBuilder.selectRightAttribute(VIRTUAL_B_BA + ".BOOKS", "PUBLISHER");
			criteriaBuilder.selectLeftAttribute(VIRTUAL_A_P + ".PUBLISHERS", "PUBLISHER_ID");
	...
	public void test09() {
		try {
			...
			this.authorAttribute = firstAuthorRaw.substring(0, firstAuthorRaw.indexOf(" "));// author* : string -->
																							// author*
	...
	public void test10() {
		try {
			ModelExplorer mew = TeiidPerspective.getInstance().getModelExplorerView();
	...
	public void test11() {
		try {
			...
			criteriaBuilder.selectRightAttribute(VIRTUAL_B_BA + ".BOOK_AUTHORS", "ISBN");
			criteriaBuilder.selectLeftAttribute("INPUTS", "isbn");
			criteriaBuilder.apply();

			new PushButton("AND").click();

			criteriaBuilder.selectRightAttribute(VIRTUAL_B_BA + ".BOOK_AUTHORS", "AUTHOR_ID");
			...
			String newSelect = originalSql.replaceFirst(
	...
	/**
	 * Refactor models --- NO, will end in mess (at least 1 method uses as path project-model-table, no folders... Edit
	 * choice -- nejde, criteria builder
	 */
	@Test
	public void test12() {
		try {
	...
	public void test15() {
		try {
			...
			editor.showTransformation();
			editor.setFocus();
			editor.setTransformationProcedureBody(VIRTUAL_PROCEDURE_SQL, true);// WHERE BooksInfo.bookInfo.EDITION =
																				// editionIn
			editor.save();
	...
	public void test16() {
		try {
			...
			editor.showTransformation();
			modelView.addTransformationSource(PROJECT_NAME, BOOKSINFO_MODEL + ".xmi", BOOKINFO_VIRTUAL_TABLE_);
			editor.setTransformationProcedureBody(VIRTUAL_PROCEDURE_SQL2, true);// WHERE BooksInfo.bookInfo.EDITION =
																				// editionIn
			editor.save();
		} catch (Exception ex) {
			// do it manually
