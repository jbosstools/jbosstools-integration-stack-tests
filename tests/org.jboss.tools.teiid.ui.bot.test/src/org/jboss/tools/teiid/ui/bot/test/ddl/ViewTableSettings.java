...
public class ViewTableSettings {
	@InjectRequirement
	...
	private static final String PROJECT_NAME = "ViewTableSettings";
	private static final String NAME_VIEW_MODEL = "ViewTableSettings";
	private static final String NAME_VDB = "ViewTableSettingsVDB";
	private static final String NAME_ORIGINAL_DYNAMIC_VDB  = NAME_VDB + "-vdb.xml";
	
	private static final String NAME_GENERATED_DYNAMIC_VDB = "viewTableSettingsVDBGenerated-vdb.xml";
	...
	private void checkExportedFile(String contentFile){
		collector.checkThat("missing name in source", contentFile, new StringContains("NAMEINSOURCE 'myTableSource'"));
		collector.checkThat("missing materialized true/false", contentFile,	new StringContains("MATERIALIZED 'TRUE'"));
		collector.checkThat("missing updatable", contentFile, new StringContains("UPDATABLE 'TRUE'"));
		collector.checkThat("missing cardinality",contentFile, new StringContains("CARDINALITY '120'"));
		collector.checkThat("missing materialized table", contentFile, new StringContains("MATERIALIZED_TABLE 'ViewTableSettings.helpTable'"));
		collector.checkThat("missing table description", contentFile, new StringContains("ANNOTATION 'Table description'"));
		collector.checkThat("missing SQL transformation", contentFile, new StringContains("'test' AS Column1;"));
	}
}
