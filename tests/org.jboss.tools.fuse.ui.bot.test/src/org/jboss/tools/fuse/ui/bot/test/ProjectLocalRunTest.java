...
/**
 * Tests option <i>Run a Project as Local Camel Context (with or without tests)</i>.
 * The option is tested on a project from archetype <i>camel-archetype-spring</i>.
 * If you want to change archetype (see static variables), you have to change
 * <i>resources/FailingTest.java</i> and <i>resources/PassingTest.java</i> too
 * (to correspondent with a new project).
 * 
 * @author tsedmik
 */
@CleanWorkspace
...
public class ProjectLocalRunTest extends RedDeerTest {
	...
	public void runProjectWithPassingTestsTest() {
		...
		assertFalse("This build should be successful.",
				new ConsoleView().getConsoleText().contains("BUILD FAILURE"));
	}
