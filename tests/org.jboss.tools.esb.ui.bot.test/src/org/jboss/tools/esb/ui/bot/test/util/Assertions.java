...
public class Assertions {
	...
	public static void assertButtonEnabled(Button button, boolean enabled) {
		if (enabled) {
			assertTrue(button.getText() + " button is not enabled when all required fields are filled",
					button.isEnabled());
		} else {
			assertFalse(button.getText() + " button is not enabled when all required fields are filled",
					button.isEnabled());
		}
	...
	public static void assertXmlContentBool(String xml, String xpath) {
		...
		assertTrue("XML content was not expected by xpath '" + xpath + "'\n instead content was\n" + xml,
				validator.executeBoolean(xpath));

	}

	public static void assertXmlContentString(String xml, String xpath, String expected) {
		XmlFileValidator validator = null;
		try {
			validator = new XmlFileValidator(xml);
		} catch (Exception e) {
		...
		assertTrue("Expected content was '" + expected + "', selected was '" + selected + "' - xpath '" + xpath
				+ "'\n XML instead content was\n" + xml, expected.equals(selected));
	...
	public static void assertEmptyProblemsView(String msg) {
		...
		problemsView.open();
		assertTrue("ESB Editor has errors in 'Problems View' - " + msg,
