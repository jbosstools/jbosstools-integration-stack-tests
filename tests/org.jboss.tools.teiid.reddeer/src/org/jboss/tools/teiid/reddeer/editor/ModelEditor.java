...
public class ModelEditor extends SWTBotEditor {
	...
	/*public void clickOnRecursiveButton(String mappingClass){
		viewer = getGraphicalViewer(MAPPING_DIAGRAM);
		
		RecursiveButtonMatcher m = new RecursiveButtonMatcher();
		List<SWTBotGefEditPart> list = viewer.editParts(m);
		...
		public boolean matches(Object item) {
			...
			if (item instanceof GraphicalEditPart) {
				if (item.getClass()
					...
					List<IFigure> children = figure.getChildren();//header, footer, triangle, imagefigure, button
					for (IFigure figure2 : children){
			
						if (figure2 instanceof Clickable){
							button =(Clickable) figure2; 
