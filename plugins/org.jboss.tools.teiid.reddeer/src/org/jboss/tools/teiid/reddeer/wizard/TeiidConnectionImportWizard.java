...
public class TeiidConnectionImportWizard extends ImportWizardDialog {
	 ...
	 * @param modelName
	 * @param type
	 *            of data source (e.g. SQLSERVER)
	 * @param properties
	 *            path to properties file of data source
	 * @param projectName
	 * @param tables
	 *            to be imported
	 */
	/*
	 * public void createNewDataSource(String dataSourcename, String modelName, String type, String properties, String
	 * projectName, List<String> tables){ //-----first page
	 * 
	 * //Create datasource new PushButton("New...").click(); new LabeledText("Name:").setText(dataSourcename);
	 * Properties props = loadProperties(properties); //set driver Table table = new DefaultTable(0); int
	 * originalRowCount = table.rowCount();
	 * 
	 * //default drivers for (int i = 0; i < originalRowCount; i++){ if
	 * (table.getItem(i).getText(0).equals(props.getProperty("db.driver"))){ table.select(i); break; } }
	 * 
	 * 
	 * new WaitWhile(new IsInProgress(), TimePeriod.NORMAL);
	 * 
	 * 
	 * if (type.equals(SQLSERVER_TYPE)){ addDataSourceSQLServer(props); }
	 * 
	 * new PushButton("OK").click(); new PushButton("&Next >").click();
	 * 
	 * //-----second page
	 * 
	 * 
	 * //Select (the translator) and target model for the import new LabeledText("Name:").setText(modelName); new
	 * PushButton("&Next >").click(); new WaitWhile(new IsInProgress(), TimePeriod.LONG);
	 * 
	 * //Get DDL for the Import new PushButton("&Next >").click();
	 * 
	 * //Review Model Updates new PushButton("Unselect All").click(); //choose tables to import
	 * checkTablesToImport(projectName, modelName, tables); new PushButton("Finish").click(); }
	 */
	 ...
	 */
	/*
	 * public void addDataSourceSQLServer(Properties props) { //set connection info Table table = new DefaultTable(1);
	 * 
	 * for (int i = 0; i < table.rowCount(); i++){ //connection url if (table.getItem(i).getText(0).equals(
	 * "* connection-url")){ table.select(i); SWTBotTable t = new SWTWorkbenchBot().table(1); t.doubleClick(i, 1);
	 * String url = "jdbc:sqlserver://"+props.getProperty("db.hostname")
	 * +":1433;databasename="+props.getProperty("db.name"); new SWTWorkbenchBot().text(1).setText(url); } //username if
	 * (table.getItem(i).getText(0).equals("user-name")){ table.select(i); SWTBotTable t = new
	 * SWTWorkbenchBot().table(1); t.doubleClick(i, 1); new
	 * SWTWorkbenchBot().text(1).setText(props.getProperty("db.username")); } //password if
	 * (table.getItem(i).getText(0).equals("password")){ table.select(i); SWTBotTable t = new
	 * SWTWorkbenchBot().table(1); t.doubleClick(i, 1); new
	 * SWTWorkbenchBot().text(1).setText(props.getProperty("db.password"));
	 * 
	 * //click somewhere else table.select(i+1); } } }
	 */

	/**
	 * 
	 * @param properties
	 *            path to file with properties of data source
	 * @return properties of data source
	 */
	/*
	 * public Properties loadProperties(String properties){ // load properties Properties props = new Properties(); try
	 * { props.load(new FileReader(properties)); } catch (FileNotFoundException e) { e.printStackTrace(); } catch
	 * (IOException e) { e.printStackTrace(); } return props; }
	 */
	 ...
	 */
	/*
	 * public void checkTablesToImport(String projectName, String modelName, String[] tables){ String wholeModelName =
	 * "file:/" + projectName + "/" + modelName + ".xmi"; for (String table : tables){ new
	 * SWTWorkbenchBot().tree().expandNode(wholeModelName, table).check(); } }
	 */
