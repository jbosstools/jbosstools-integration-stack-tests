...
public class ElementContainer extends Element {
	...
	public Element addRelativeToElement(String name, ElementType type, Element nextTo, Point move) {
		return addToAbsolutePoint(name, type,
				new Point(nextTo.getBounds().getCenter().x + move.x, nextTo.getBounds().getCenter().y + move.y));
	}
	 ...
	 * @param type
	 * @param point
	 *            Represents a position in the canvas relative to this construct. Meaning that this construct's upper
	 *            left corner is the staring position.
	 */
	...
	/**
	 * Check weather ${p} located in this container construct is not covered by another construct.
	 * 
	 * @param point
	 *            a absolute posiiton on canvas
	 * @return
	 */
